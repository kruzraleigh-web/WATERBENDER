...
function spawnEnemy() {
  // Each wave has a fixed enemy budget, and enemies vary in size + speed.
  ...
  const size = rand(10, 21);
  const speed = rand(55, 105) + state.wave * 3 - size * 1.1;
  const hp = 24 + size * 1.3 + state.wave * 2;
  state.enemies.push({
    x, y, r: size, hp, speed: clamp(speed, 45, 160),
...
function releaseDrag() {
  ...
  // Stop dragging without auto-shooting. This keeps the mechanic focused on sliding/whipping.
  addParticles(player.orbX, player.orbY, state.dragType === "ice" ? "#d7f8ff" : "#66dcff", 8, 110);
}
...
function updatePlayer(dt) {
  ...
  // Orb no longer spins around the player.
  // When not dragging, it rests on the side of the player facing the mouse.
  if (!state.draggingOrb) {
    const dx = state.mouse.x - player.x;
    const dy = state.mouse.y - player.y;
    const d = Math.hypot(dx, dy) || 1;
    player.orbX = player.x + (dx / d) * player.orbRadius;
    player.orbY = player.y + (dy / d) * player.orbRadius;
    return;
  }
...
function updateEnemies(dt) {
  ...
  const spawnInterval = clamp(1.8 - state.wave * 0.08, 0.95, 1.8);
  const aliveCap = 2 + Math.floor(state.wave / 2);
  if (state.waveDefeated + state.enemies.length < state.waveTarget && state.enemies.length < aliveCap) {
    state.spawnTimer -= dt;
...
function drawHumanoid(x, y, color, phase, scale = 1) {
  const leg = Math.sin(phase) * 3 * scale;
  const arm = Math.cos(phase) * 3 * scale;
  ctx.fillStyle = color;
  ctx.beginPath();
  ...
  ctx.lineCap = "round";
  ctx.beginPath();
  ctx.moveTo(x, y - 7 * scale); ctx.lineTo(x, y + 10 * scale);
  ctx.moveTo(x, y - 2 * scale); ctx.lineTo(x - 9 * scale, y + 4 * scale + arm);
  ctx.moveTo(x, y - 2 * scale); ctx.lineTo(x + 9 * scale, y + 4 * scale - arm);
  ctx.moveTo(x, y + 10 * scale); ctx.lineTo(x - 6 * scale, y + 22 * scale + leg);
  ctx.moveTo(x, y + 10 * scale); ctx.lineTo(x + 6 * scale, y + 22 * scale - leg);
  ctx.stroke();
}
...
function drawOrbAndWhip() {
  ...
  ctx.arc(player.orbX, player.orbY, 10 + Math.sin(player.walkAnim * 1.8) * 1.8, 0, Math.PI * 2);
  ctx.fill();
}
...
function draw() {
  ...
  for (const e of state.enemies) {
    ...
    drawHumanoid(e.x, e.y, color, e.walkAnim, e.r / 14);
  }
...
function resetGame() {
  ...
  Object.assign(state, {
    keys: {}, draggingOrb: false, rightDown: false, enemies: [], particles: [], waveRings: [],
    structures: [], score: 0, xp: 0, level: 1, wave: 1, waveTarget: 4, waveDefeated: 0,
